...
class Poll(commands.Cog):
    ...
    async def encuesta(self, ctx, max_time: int, max_votes: int, titulo, *options):
        ...
        """
        if len(options) > 10:
            await ctx.send(':no_entry: El maximo permitido son 10 opciones ')

        elif max_time <= 15:
            await ctx.send(':no_entry: La duracion minima de una encuesta debe superar los **15 minutos**!')

        elif 1000000 < max_votes:
            await ctx.send(':no_entry: No es posible crear una encuesta con votos minimos por encima de un millon .___. **million**!')
            ...
            with open('.\\databases\\scheduler.json', 'r') as scheduler_file:
                ...
                if str(ctx.message.channel.id) not in scheduler_data:
                    ...
                    embed = discord.Embed(title=titulo,
                                          description=f':stopwatch: La encuesta terminara en **{max_time} minutos**!',
                                          colour=0xFF0000)
                    ...
                    # Poll schedule
                    scheduler_data[message.channel.id] = {'message_id': message.id, 'scheduler_time': max_time,
